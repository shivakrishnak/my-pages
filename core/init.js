...
(function () {
    ...
    } catch (e) { /* localStorage unavailable — ignore, default light */ }
    ...
    var ICON_MENU = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>';
    var ICON_HOME = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>';
    var ICON_TOP = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>';
    var ICON_DARK = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>';
    ...
    function buildLayout() {
        ...
        var title = body.getAttribute('data-title') || document.title || 'Untitled Page';
        ...
        Array.from(document.body.querySelectorAll('body > script, body > * > script')).forEach(function (s) {
        ...
        progress.innerHTML = '<div class="ms-progress-bar" id="ms-progress-bar"></div>';
        ...
        if (layout === 'full') {
            ...
            hamburgerHtml =
                '<button class="ms-hamburger" id="ms-hamburger" aria-label="Open navigation" aria-expanded="false" aria-controls="ms-sidebar">' + ICON_MENU + '</button>';

            var tocHtml = '<span class="ms-nav-label">This Page</span>';
            if (buildToc) {
                tocHtml += '<div id="ms-auto-toc"></div>';
            } else {
                tocHtml += '<span style="display:block;padding:6px 1.1rem;font-size:11.5px;color:var(--text4);line-height:1.5">No table of contents configured. Add data-toc="true" to &lt;body&gt; for an automatic one, or hand-write sidebar links — see core/templates/template-with-sidebar.html.</span>';
            }

            sidebarHtml =
                '<aside class="ms-sidebar" id="ms-sidebar" aria-label="Site navigation">' +
                '<div class="ms-sidebar-header">' +
                '<a href="' + rootBase + 'index.html" class="ms-brand">' + escapeHtml(brand) + '<span>Knowledge Hub</span></a>' +
                ...
                '</div>' +
                '<nav class="ms-toc" aria-label="Page sections">' + tocHtml + '</nav>' +
                '<div class="ms-sidebar-footer"><a href="' + rootBase + 'index.html">← All pages</a></div>' +
                '</aside>';
        }

        var badgeHtml = badge ? '<span class="ms-badge ' + escapeHtml(badgeColor) + '">' + escapeHtml(badge) + '</span>' : '';

        var topbarHtml =
            '<header class="ms-topbar">' +
            '<div class="ms-topbar-left">' +
            hamburgerHtml +
            (layout === 'mixed' ? '<a href="' + rootBase + 'index.html" style="font-size:13px;font-weight:700;color:var(--text1);text-decoration:none;flex-shrink:0">← ' + escapeHtml(brand) + '</a>' : '') +
            badgeHtml +
            '<span class="ms-topbar-title">' + escapeHtml(title) + '</span>' +
            '</div>' +
        ...
        var bottomNavHtml =
            ...
            (layout === 'full'
                ? '<button class="ms-bottom-nav-item" data-action="menu" aria-label="Open menu">' + ICON_MENU + '<span>Menu</span></button>' +
                '<a class="ms-bottom-nav-item" href="' + rootBase + 'index.html" aria-label="Home">' + ICON_HOME + '<span>Home</span></a>'
                : '<a class="ms-bottom-nav-item" href="' + rootBase + 'index.html" aria-label="Home">' + ICON_HOME + '<span>Home</span></a>') +
            '<button class="ms-bottom-nav-item" data-action="top" aria-label="Back to top">' + ICON_TOP + '<span>Top</span></button>' +
            '<button class="ms-bottom-nav-item" data-action="dark" aria-label="Toggle dark mode">' + ICON_DARK + '<span>Dark</span></button>' +
            '</nav>';
        ...
        layoutDiv.innerHTML =
            sidebarHtml +
            '<main class="' + mainClass + '" id="main-content">' +
            topbarHtml +
            '<div class="ms-content">' + contentHtml + '</div>' +
            bottomNavHtml +
            '</main>';
        ...
        if (layout === 'full' && buildToc) {
            ...
            if (tocTarget) {
                var headings = document.querySelectorAll('.ms-content h2, .ms-content h3');
                var html = '';
                var found = 0;
                headings.forEach(function (h) {
                    ...
                    html += '<a href="#' + targetId + '" class="ms-nav-item sub">' + prefix + escapeHtml(h.textContent) + '</a>';
                });
        ...
        if (pageScripts.length) {
            function runNextScript(index) {
                ...
                if (info.src) {
                    s.src = info.src;
                    s.onload = function () { runNextScript(index + 1); };
                } else {
    ...
    function run() {
        buildLayout();
        var layout = (document.body.getAttribute('data-layout') || 'full').toLowerCase();
        if (layout !== 'none') {
            loadShellJs();
        }
    }
